/**
 * Authenticates a user to Sign in with World using a World ID ZKP.
 */
import "server-only";
...
export const authenticateSchema = yup.object({
  ...
  state: yup
    ...
    .matches(/^[a-zA-Z0-9,._+-/=]{0,1024}$/, {
      message:
        "State parameter must be 1024 characters or less and contain only alphanumeric, comma, period, underscore, and hyphen characters.",
    }),
